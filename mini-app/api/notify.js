/**
 * Notification endpoint — called by frontend to send Telegram messages
 */
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'new_lead' && new_lead) {
      const { full_name, offer, revenue, payout, added_by } = new_lead
      await broadcast(
      ...
      const { full_name, offer, new_status, changed_by } = status_change
      await broadcast(
        `🔄 <b>Статус изменён</b>\n` +
        `👤 ${full_name}\n` +
        `📋 Оффер: ${offer}\n` +
